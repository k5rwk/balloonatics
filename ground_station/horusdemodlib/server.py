            ...
            if flag & (select.POLLIN | select.POLLPRI):
                ...
                if p is s: # server
                    try:
                        ...
                        # TEMPORARY LOCAL WORKAROUND (pending upstream fix):
                        # another worker won the accept race; nothing left to
                        # accept on the non-blocking listen socket, so go back
                        # to polling our own client sockets instead of blocking.
                        continue
...
def main():
    ...
    # TEMPORARY LOCAL WORKAROUND (pending upstream fix):
    # The pool's workers all poll() this single inherited listening socket
    # (level-triggered), so a new connection wakes every idle worker and they
    # all race into accept(). With a BLOCKING socket the losers stall forever
    # in accept(), abandoning the client sockets they already own -> channels
    # silently go dark at random. Making the listen socket non-blocking lets
    # the race losers get EAGAIN and return to their poll loop (see the
    # BlockingIOError guard around accept() in worker()).
    s.setblocking(False)
